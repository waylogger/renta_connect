...
import { getSavedSecret, secretFile, saveSecret, getToken, auth } from './app';
...
describe('testing toprent15.ru API', () => {
	...
	test('test getToken', async () => {
		...
		expect(await getToken(badAccessData)).toBeUndefined();
	});
	test('test auth', async () => {
		expect(await auth(secretFile,goodAccessData)).toBe(codes.authSuccess);
		expect(await auth(noExist,badAccessData)).toBe(codes.authFailure);
	});
